...
### No HAL, No CubeMX Peripheral Init

All peripheral drivers (USART2, I2C1, TMP117, MAX30102, IWDG) are written from RM0351 using CMSIS device headers (`stm32l476xx.h`). No STM32CubeMX HAL. No CubeMX-generated peripheral init code.

`syscalls.c` and `sysmem.c` are newlib system call stubs generated by STM32CubeIDE and retained unchanged. They are not peripheral drivers and contain no project-specific code.
